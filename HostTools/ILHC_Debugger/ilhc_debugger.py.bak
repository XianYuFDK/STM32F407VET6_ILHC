...
class FrameParser:
    ...
    支持任意分包/粘包，以 JustFloat 固定帧尾重新同步。
    """
    ...
    def feed(self, data):
        """输入任意长度字节流，返回解析出的 [(t 无关) float 元组, ...]"""
        self.buf += data
        self.bytes_in += len(data)
        frames = []
        while True:
            i = self.buf.find(FRAME_TAIL)
            if i < 0:
                # 保留一帧数据和可能被截断的帧尾
                ...
                break
            if i < FRAME_DATA_LEN:
                self.err_bytes += i + len(FRAME_TAIL)
                del self.buf[:i + len(FRAME_TAIL)]
                continue

            start = i - FRAME_DATA_LEN
            if start > 0:
                self.err_bytes += start
            values = struct.unpack("<%df" % FRAME_FLOATS,
                                   bytes(self.buf[start:i]))
...
class SerialWorker(threading.Thread):
    def __init__(self, port, baud, frame_q, line_q, err_cb=None):
...
class Simulator(threading.Thread):
    def __init__(self, frame_q, line_q):
        super().__init__(daemon=True)
        self.frame_q, self.line_q = frame_q, line_q
        self.stop_flag = False
        # 底盘参数（与固件默认值一致）
...
class App:
    ...
    def _build_dm_tab(self):
        ...
        for (cmd, label, lo, hi, dflt, rb, is_int) in DM_PARAMS:
            self._make_param_row(body, cmd, "%s %s" % (cmd, label), lo, hi, dflt,
                                 rb, self.dm_rows)
            if is_int:
                self.dm_rows[cmd]["var"].trace_add(
                    "write", lambda *a, c=cmd: self._dm_id_int(c))
    ...
    def toggle_connect(self, force_on=None):
        ...
        if want and self.worker is None:
            ...
            self.worker = SerialWorker(port, baud, self.frame_q, self.line_q,
                                       err_cb=self._worker_err)
            self.worker.start()
            self.btn_conn.configure(text="  断开  ", bg="#8d6e63")
            self.lbl_status.configure(text="● 已连接 %s @ %d" % (port, baud), fg="#81c784")
            self.log("串口已连接：%s @ %d 8N1" % (port, baud), "info")
    ...
    def toggle_sim(self, force_on=None):
        ...
        if want and self.sim is None:
            ...
            self.sim = Simulator(self.frame_q, self.line_q)
            self.sim.start()
