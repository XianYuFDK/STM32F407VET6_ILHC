...
# ---------------- 4) OPS 原始帧到统一坐标的唯一映射 ----------------
# 实车原始帧：+x_raw=车右、+y_raw=车头；统一映射为 X=-x_raw、Y=+y_raw。
must(o, "*x = -px; *y = py;", "原始帧位置映射")
must(o, "*z = zero ? (yaw - zero_yaw) : yaw;", "ZERO 同时归零航向")
must(o, "rx = -s_mount_x_mm * 0.001f;", "统一 X 偏移 → 原始 x")
must(o, "ry = s_mount_y_mm * 0.001f;", "统一 Y 偏移 → 原始 y")
must(o, "float dx = dc * rx - ds * ry;", "偏心矩阵 x")
must(o, "float dy = ds * rx + dc * ry;", "偏心矩阵 y")
must(o, "*x = -(px - ox) + dx;", "置零后 X")
must(o, "*y = (py - oy) - dy;", "置零后 Y")
must(o, "*x = -px + dx;", "未置零 X")
must(o, "*y = py - dy;", "未置零 Y")
must(o, "s_mount_x_mm = 60.0f;", "默认安装 X=车左60mm")
must(o, "s_mount_y_mm = -50.0f;", "默认安装 Y=车后50mm")
must(o, "s_ops.origin_x = -x;", "SetOrigin X 转原始 x")
must(o, "s_ops.origin_y = y;", "SetOrigin Y 转原始 y")
...
must(m, "cmd_x = mKpx * ( c * devx + s * devy);", "P 增益在车体坐标应用 X")
must(m, "cmd_y = mKpy * (-s * devx + c * devy);", "P 增益在车体坐标应用 Y")
must_not(m, "c * mKpx * devx", "旧 X P 在旋转前应用")
must_not(m, "s * mKpy * devy", "旧 Y P 在旋转前应用")
...
print("  OPS raw frame maps as X=-raw_x, Y=+raw_y with mount (60,-50)mm")
